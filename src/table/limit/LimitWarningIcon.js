...
export default function LimitWarningIcon(): React$Node {
  const [node, setNode] = useState(null);
  useLayoutEffect(() => warn(node), [node]);
  return (
    <svg
      onClick={() => warn(node)}
      ref={setNode}
...
function warn(node) {
  if (node)
    createConfirmTooltip(node, {
      contents:
        'Your request exceeded the limit! You are working with a partial result.',
      level: 'warning',
      confirmLabel: 'Got it',
      cancelLabel: null,
    });
